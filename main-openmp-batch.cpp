...
int main(int argc,char* argv[])
{
	...
	for(int i=0;i<n_layers-1;i++)
	{
		...
		for(int j=0;j<layer[i];j++)
		{
			...
			for(int k=0;k<layer[i+1];k++)
				d_w[i][j][k]=((double)rand()/(RAND_MAX))/n_images_train;
		}
	}
...
#	pragma omp parallel for num_threads(THREAD_COUNT) shared(input_train,n_images_train,n_layers,layer,b,w,output_train,error_train,d_w,d_b) private(z,instance_error) schedule(dynamic)
...
#	pragma omp parallel for num_threads(THREAD_COUNT) shared(n_images_test,input_test,n_layers,layer,b,w,output_test,error_test,d_w,d_b) private(z,instance_error) schedule(dynamic)
...
#	pragma omp parallel for num_threads(THREAD_COUNT) shared(layer,b,alpha,d_b,w) schedule(dynamic)
