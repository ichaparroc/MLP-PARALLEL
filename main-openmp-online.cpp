...
int main(int argc,char* argv[])
{
	...
	for(int epoch=0;epoch<n_epoch;epoch++)
	{
		...
		for(int instance=0;instance<n_images_train;instance++)
		{
			...
			for(int i=0;i<n_layers-1;i++)
#	pragma omp parallel for num_threads(THREAD_COUNT) shared(i,layer,w,a,b) private(z) schedule(dynamic)
				for(int j=0;j<layer[i+1];j++)
				{
					z=b[i][j];
...
#	pragma omp parallel for num_threads(THREAD_COUNT) shared(layer,d_b,output_train,a) schedule(dynamic)
...
#	pragma omp parallel for num_threads(THREAD_COUNT) shared(layer,w,d_b) private(z) schedule(dynamic)
				for(int i=0;i<layer[m+1];i++)
				{
					z=0.0;
			...
			//new weight and bias
#	pragma omp parallel for num_threads(THREAD_COUNT) shared(n_layers,layer,b,alpha,d_b,w,a) schedule(dynamic)
